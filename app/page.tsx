  ...
  () => import('@/components/portfolio/PortfolioCanvas'),
  {
    ...
    loading: () => (
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-slate-300 text-sm tracking-widest animate-pulse">Loading...</span>
      </div>
    ),
  }
)
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      setChapter(prev => {
        ...
        if (e.deltaY > 0 && idx < CHAPTERS.length - 1) return CHAPTERS[idx + 1].key
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 py-4">
        <span className={`font-bold text-base tracking-tight ${is3DMode ? 'text-slate-100' : 'text-slate-700'}`}>JUYOUNG</span>
